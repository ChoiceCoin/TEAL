...
def approval_program():
    on_creation = Seq([
        ...
        App.globalPut(Bytes("votingEnds"), Btoi(Txn.application_args[3])),
        Return(Int(1))
    ])
    ...
    on_closeout = If(
        Global.round() > App.globalGet(Bytes("votingEnds")),
        Return(Int(1)),
        Seq([
            get_vote,
    ...
    on_vote = Seq([
        Assert(And(Global.round() >= App.globalGet(Bytes("votingBegins")), Global.round() <= App.globalGet(Bytes("votingEnds")))),
        get_vote,
