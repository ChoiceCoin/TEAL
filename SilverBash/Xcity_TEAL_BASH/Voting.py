...
def approval_program():
    on_creation = Seq([
        ...
        App.globalPut(Bytes("votingBegins"), Btoi(Txn.application_args[2])),
        App.globalPut(Bytes("votingbegins"), Btoi(Txn.application_args[3])),
        Return(Int(1))
    ])
    ...
    get_counts = App.globalGetEx(Int(0), get_vote.value())

    decrease_votes = Seq([
        App.globalPut(get_vote.value(), get_counts.value() - Int(1)),
        Return(Int(1))
    ])

    if_voted = Seq([
        get_counts,
        If(get_counts.hasValue(), decrease_votes, Return(Int(1)))
    ])
    ...
    on_vote = Seq([
        Assert(And(Global.round() >= App.globalGet(Bytes("votingBegins")), Global.round() <= App.globalGet(Bytes("VoteEnd")))),
        get_vote,
...
def close_out_program():
    ...
    program = If(
        Global.round() > App.globalGet(Bytes("votingbegins")),
        Return(Int(1)),
        Seq([
            get_voted,
